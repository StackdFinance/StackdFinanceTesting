...
const lp_abi = lp.abi;
const vesting_instance = JSON.parse(fs.readFileSync('./build/contracts/StackdTeamVestingInstance.json', 'utf8'));
...
contract('Stacked Finance Team Vesting Test', async accounts => {
    ...
    it('Can Claim Vested Tokens And Dividends', async() => {
        ...
        await timeMachine.advanceTime(1209600);
        await timeMachine.advanceBlock();
        block = await web3.eth.getBlock('latest');
    ...
    it('10 more weeks', async() => {
        let block = await web3.eth.getBlock('latest');
        console.log("Block Time Before: ", block.timestamp)
        await timeMachine.advanceTime(6048000);
        await timeMachine.advanceBlock();
        block = await web3.eth.getBlock('latest');
        console.log("Block Time After: ", block.timestamp)
        let instances = await vestingManager.getAllInstances();
    ...
    it('Cancel Vesting Instance', async() => {
        let instBefore = await vestingManager.getAllInstances();
        console.log("List Before Cancel", instBefore);
        let balBefore = await stacked.balanceOf(accounts[0]);
        console.log("Owner Bal Before", balBefore.toString());

        await vestingManager.cancelVestingInstance(instBefore[0]);

        let instAfter = await vestingManager.getAllInstances();
        console.log("List After Cancel", instAfter);
        balBefore = await stacked.balanceOf(accounts[0]);
        console.log("Owner Bal After", balBefore.toString());

        let newIndex = await vestingManager.idToInstance(0);
